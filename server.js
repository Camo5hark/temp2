...
https.createServer({
    ...
    console.log(`${req.socket.remoteAddress.substring(7)}:${req.socket.remotePort} ${req.url}`)
        ...
        case "/stream":
            if (hasAccess(req, res)) {
                ...
                for (var i = 0; i < files.length; i++) {
                    videos += `<li class="video" onclick="onclickVideos(${i})">${files[i].substring(0, files[i].length - 4)}</li>`
                }

                res.writeHead(200, {"Content-Type": "text/html"}).end(fs.readFileSync("html/stream.html").toString().replace(/%VIDEOS%/, videos))
